...
class BrowserTest(BaseTest):
    ...
    def scan(self, password=None, error=None):
        ...
        if password:
            ...
            self.app.child(roleName='text entry', label='Encryption password').typeText(password)
    ...
    def assert_search_mode(self, searching=True):
        ...
        search_entry = self.app.findChild(GenericPredicate(roleName='text', name='Search'),
    ...
    def assert_selection(self, selecting=True):
        predicate = GenericPredicate(roleName='toggle button', name='Click on items to select them')
        selection_button = self.app.findChild(predicate, retry=False, requireResult=False)
        if selecting:
    ...
    def select_location(self, where):
        ...
        self.window.child(roleName='radio button', name='Restore to specific folder').click()
    ...
    def walk_restore(self, password=None, error=False, where=None):
        ...
        if password:
            self.window.child(roleName='text', label='Encryption password').text = password
            self.window.button('Forward').click()

        title = 'Restore Failed' if error else 'Restore Finished'
        self.wait_for(
            lambda: self.window.findChild(GenericPredicate(name=title),
                                          retry=False, requireResult=False),
            timeout=60,
        )
    ...
    def select(self, *args):
        ...
        view = self.app.findChild(lambda x: x.roleName == 'layered pane', retry=False, requireResult=False)
        role = 'icon'
        check_selected = True
        if not view:
        ...
        children = view.findChildren(lambda x: x.roleName == role and x.text, showingOnly=False)
    ...
    def test_selection(self):
        ...
        menu = self.app.child(roleName='toggle button', name='Click on items to select them')
        menu.click()
    ...
    def test_encrypted_and_dates(self):
        ...
        dir1 = self.app.findChild(lambda x: x.roleName == 'icon' and x.text == 'dir1', showingOnly=False)
        dir1.doubleClick()
        ...
        dates = [
            x.name for x in
            dates_combo.findChildren(lambda x: x.roleName == 'menu item', showingOnly=False)
        ]
    ...
    def test_bad_restore_permissions(self):
        ...
        def findPermissionLabel(x):
            return x.roleName == 'label' and x.name == 'Backups does not have permission to restore the following files:'
        ...
        self.app.findChild(lambda x: x.roleName == 'label' and x.name == self.srcdir + '/four.txt')
        assert not button.sensitive

        # Test that we can if you switch locations
        self.select_location(self.srcdir)
        label = self.app.findChild(findPermissionLabel, retry=False, requireResult=False)
