...
class BrowserTest(BaseTest):
    def setUp(self):
        ...
        self.set_boolean('root-prompt', False)

        basedir = os.path.realpath(os.path.join(os.path.dirname(__file__)))
        self.set_string('folder', basedir + '/unencrypted', child='local')
    ...
    def walk_restore(self, password=None, error=False, where=None):
        ...
        window = self.app.window('Restore')

        shutil.rmtree(self.srcdir, ignore_errors=True)

        if where:
            self.addCleanup(shutil.rmtree, where, ignore_errors=True)
            window.child(roleName='radio button', name='Restore to specific folder').click()
            window.child(roleName='push button', label='    Restore folder').click()
            window.child(roleName='menu item', name='Other…').click()
    ...
    def test_encrypted_and_dates(self):
        basedir = os.path.realpath(os.path.join(os.path.dirname(__file__)))
        self.set_string('folder', basedir + '/encrypted', child='local')
